		...
		do {
			if (m_map.walls[x][y][1] > 0) {
				...
				if (m_map.walls[x][y][1] == 1) {
					y++;
					while (y <= m_map.y_width && m_map.walls[x][y][1] == 1 && (m_map.walls[x][start][0] == 0 || (x > 0 && m_map.walls[x - 1][start][0] == 0))) {
						y++;
					}
				}
				...
				// TO PREVENT WALLS FROM OVERLAPPING

				if (m_map.walls[x][start][0] > 0 || (x > 0 && m_map.walls[x - 1][start][0] > 0))// Next to at least 1 horizontal wall
					vect_start.Y = start * 100.0 + m_shift;
				else { // No horizontal wall
					if (start > 0 && m_map.walls[x][start - 1][1] > 0) // it's next to another vertical wall
						vect_start.Y = start * 100.0;
					else
						vect_start.Y = start * 100.0 - m_shift;
				}

				if (m_map.walls[x][end][0] > 0 || (x > 0 && m_map.walls[x - 1][end][0] > 0)) // Next to at least 1 horizontal wall
					vect_end.Y = end * 100.0 - m_shift;
				else { // No horizontal wall
					if (m_map.walls[x][end][1] > 0) // it's next to another vertical wall
	...
	for (int y = 0; y <= m_map.y_width; y++) { // We create horizontal walls
		...
		do {
			if (m_map.walls[x][y][0] > 0) {
				...
				if (start > 0 && m_map.walls[start - 1][y][0] > 0) // it's next to another wall in the same direction
					vect_start.X = start * 100.0;
				else
					vect_start.X = start * 100.0 - m_shift;


				if (m_map.walls[end][y][0] > 0) // it's next to another wall in the same direction
